...
def summarize_results(df, shift_range):
    ...
    # ── Selective attacker analysis ───────────────────────────────────
    # An attacker who only targets patients above a confidence threshold.
    # They compute the posterior for every patient, then cherry-pick.
    #
    # "Targeted" = patients the attacker decides to go after
    # "Correctly identified" = attacker's argmax == true shift
    #   (a patient with a Saturday surgery will have true shift penalized
    #    in the posterior; attacker's argmax will be wrong -> not in this count)
    # "% pop compromised" = correctly identified / total population
    #   This is the headline number for the paper.
    print(f"\n  SELECTIVE ATTACKER ANALYSIS")
    print(f"  (Attacker computes posterior for all patients, only attacks those above threshold)")
    print(f"  {'Conf.':>6} {'Targeted':>10} {'of pop':>7} "
          f"{'Correct':>10} {'Precision':>10} "
          f"{'Pop compromised':>16}")
    print(f"  {'-'*66}")

    for threshold in [0.50, 0.75, 0.90, 0.95, 0.99]:
        targeted = df[df['p_max_soft'] >= threshold]
        n_targeted = len(targeted)
        if n_targeted == 0:
            print(f"  >={threshold:.2f} {'0':>10} {'0.0%':>7} "
                  f"{'0':>10} {'—':>10} {'0.00%':>16}")
            continue
        n_correct = int(targeted['max_is_true'].sum())
        precision = 100.0 * n_correct / n_targeted
        pct_pop_targeted = 100.0 * n_targeted / n
        pct_pop_compromised = 100.0 * n_correct / n
        print(f"  >={threshold:.2f} {n_targeted:>10,} {pct_pop_targeted:>6.1f}% "
              f"{n_correct:>10,} {precision:>9.1f}% "
              f"{pct_pop_compromised:>10.2f}%  ({n_correct:,})")

    # ── Complement: patients safe from selective attack ────────────────
    n_safe_50 = (df['p_max_soft'] < 0.50).sum()
    n_safe_90 = (df['p_max_soft'] < 0.10).sum()
    print(f"\n  Patients with P(max) < 0.50 (safe from confident attacker): "
          f"{n_safe_50:,} ({100*n_safe_50/n:.1f}%)")
    print(f"  Patients with P(max) < 0.10 (effectively safe):              "
          f"{n_safe_90:,} ({100*n_safe_90/n:.1f}%)")
